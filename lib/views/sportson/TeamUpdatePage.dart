...
class TeamUpdatePage extends StatefulWidget {
  var updatesonid="";
  TeamUpdatePage({this.updatesonid});
...
class _TeamUpdatePageState extends State<TeamUpdatePage> {
  ...
  getdata() async
  {
    await FirebaseFirestore.instance.collection("Sportson").doc(widget.updatesonid).get().then((document){
      _tn.text = document["squadname"].toString();
      _pl.text = document["playername"].toString();
      _de.text = document["description"].toString();
      _ye.text = document["year"].toString();
      setState((){
        oldfilename=document["sportsfile"].toString();
        oldfileurl=document["imageurl1"].toString();
      });
    });
  }
      ...
      body: SingleChildScrollView(
        child: Container(
          ...
          child: Column(
            children: [
              SizedBox(height: 40,),
              (sportsfile!=null)?Image.file(sportsfile,width: 130,):
